...
class OBJViewer extends Viewer {
  ...
  override def apply(m: IslandMap): File = {
    ...
    val header =
      """####
         ...
         |####
      """.stripMargin
    val vertices: Seq[String] = buildVertices(m.mesh, m.vertexProps)
   ...
   * @param mesh the mesh containing the vertex registry used to store the ≠ vertices
   * @return a sequence of vertex description
   */
  private def buildVertices(mesh: Mesh, vProps: PropertySet): Seq[String] = {
    info("Building vertices index")
    ViewerHelpers.buildVertices(mesh, vProps) map { c => s"v ${c._1} ${c._2} ${c._3}" }
  }
