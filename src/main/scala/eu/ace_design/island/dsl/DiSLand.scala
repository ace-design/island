...
trait DiSLand {
   ...
   */
  // Elevations
  def flat(culmination: Double): Process = DistributeElevation(mapper = ElevationDistributions.distance,
  ...
  protected def heatMap(p: Property[Double], c: java.awt.Color = java.awt.Color.RED) = PDFViewer(HeatMap(p, c))
